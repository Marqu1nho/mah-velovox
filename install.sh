...
# ---------------------------------------------------------------------------
# 6. speakwrite Hammerspoon wiring.
# ---------------------------------------------------------------------------
CLI_SW="$REPO/.venv/bin/speakwrite"
if [ ! -x "$CLI_SW" ]; then
  warn "speakwrite CLI not found at $CLI_SW — skipping speakwrite wiring."
  warn "(Run 'uv sync' in $REPO to install it, then re-run this script.)"
else
  say "speakwrite CLI resolved: $CLI_SW"

  # 6a. Generate the install-time paths file for speakwrite.
  cat > "$HS_DIR/speakwrite_paths.lua" <<SWEOF
...
SWEOF
  say "Wrote $HS_DIR/speakwrite_paths.lua"

  # 6b. Symlink the lua module into ~/.hammerspoon/.
  ln -sf "$REPO/hammerspoon/speakwrite.lua" "$HS_DIR/speakwrite.lua"
  say "Linked speakwrite.lua into $HS_DIR"

  # 6c. Idempotently wire require("speakwrite") into init.lua.
  if grep -q 'require("speakwrite")' "$INIT" || grep -q "require('speakwrite')" "$INIT"; then
    say "init.lua already requires speakwrite."
  else
    printf '\nrequire("speakwrite")\n' >> "$INIT"
    say "Added require(\"speakwrite\") to $INIT"
  fi

  # 6d. Ensure speakwrite config directory exists.
  SW_CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/speakwrite"
  mkdir -p "$SW_CONFIG_DIR"
  say "speakwrite config dir: $SW_CONFIG_DIR"

  echo ""
  echo "  speakwrite permissions note:"
  echo "    • Accessibility — required for the synthetic Cmd-V paste."
  echo "      System Settings -> Privacy & Security -> Accessibility -> enable Hammerspoon."
  echo "    • Microphone — the process that opens the mic owns the system prompt."
  echo "      For a clean first-time mic permission dialog, run:"
  echo "        $CLI_SW stream"
  echo "      from Terminal.app BEFORE using the hotkey, so macOS prompts in a"
  echo "      context you can approve easily (Hammerspoon background processes"
  echo "      can suppress the dialog)."
fi
